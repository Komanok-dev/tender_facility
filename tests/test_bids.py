from tests.utils import create_test_organization, create_test_user, create_test_tender, assign_responsibility, create_test_bid, create_test_review
...
def test_publish_bid(client, db_session):
    ...
    response = client.post(
        f"/api/bids/{test_bid.id}/publish",
        headers={"Authorization": f"Bearer {token}"}
    )
...
def test_rollback_bid_version(client, db_session):
    ...
    response = client.patch(f"/api/bids/{test_bid.id}/edit", json=updated_data, headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200

    response = client.put(f"/api/bids/{test_bid.id}/rollback/1", headers={"Authorization": f"Bearer {token}"})
...
def test_approve_bid(client, db_session):
    ...
    response = client.post(
        f"/api/bids/{test_bid.id}/approve",
        headers={"Authorization": f"Bearer {token}"}
    )
...
def test_get_reviews_for_bid(client, db_session):
    ...
    client.post(f"/api/bids/{test_bid.id}/review", json=review_data, headers={"Authorization": f"Bearer {token}"})

    response = client.get(f"/api/bids/{test_bid.id}/reviews", headers={"Authorization": f"Bearer {token}"})
